...
2. **Read** the content using `get_page_text` or `read_page`
3. **Screenshot** if visual verification is needed (file trees, rendered markdown)
4. **Cite** the exact path, branch, SHA, or URL
5. **Log** the verified fact with its citation

## What Counts as Evidence
- Direct URL to a file or folder on GitHub (verified by navigation)
