...
| **D2** | **The build is still Node/Express live-front-door, with Python/FastAPI verified in Unified Staging as the target runtime.** | "Gateway = Node; Python = a target, not promoted." | **Node/Express remains the live Gateway front door.** **Python/FastAPI exists and is verified in `PHOENIX_UNIFIED_STAGING` as the target runtime**, not proven production-primary on both VPS lanes. Python promotion is unresolved and must go through the runbooks / Deep Research path before the codebase rewrite. |  
...
### 1.6 Runtime & Hosting — staging/deploy evidence, not production-primary proof  
  
| Lane | Host | Bind / service seen in deploy evidence | Latest staging deploy proof | What it proves | What it does not prove |  
|------|------|---------------------------------------|-----------------------------|----------------|------------------------|  
| **Echo VPS lane** | echo.phoenixelectric.life | 127.0.0.1:9120 / `phoenix-echo` systemd | GitHub Actions success `27665761798` (2026-06-17) | Deploy OIDC, Key Vault access, TLS configuration, HTTPS health check path, Python target service path | That Python/FastAPI is the universal production-primary front door |  
| **Firedancer VPS lane** | firedancer.phoenixelectric.life | 127.0.0.1:9120 / `phoenix-firedancer` systemd | GitHub Actions success `27313341234` (2026-06-10) | Deploy OIDC, Key Vault access, TLS configuration, HTTPS health check path, Python target service path | That Python/FastAPI is the universal production-primary front door |  
  
> **Verified 2026-06-30 from staging evidence, not by touching production.** `PHOENIX_UNIFIED_STAGING origin/main` deploy workflow uses GitHub lane environments (`staging-echo`, `staging-firedancer`) with `id-token: write` and `Azure Login (OIDC)`, verifies Key Vault access, pulls lane secrets from `phoenixaaivault`, installs/refreshes the Python systemd service, configures nginx/TLS, and runs post-deploy health checks. This proves **deploy OIDC is established** for CI -> Azure/Key Vault and that the Python target path is real. It does **not** override Shane's correction that Node/Express remains relevant until the runbooks / Deep Research rewrite path finishes. It also does **not** mean browser/user auth is OIDC; runtime auth remains `PHOENIX_TOKEN` / `X-Phoenix-Token` until changed by a later approved auth design.  
...
| **Q11** Build start timing | (was open) | Build is active, but the old production-primary Python claim is not accepted. Current wording: Python/FastAPI is verified in Unified Staging as the target runtime; Node remains relevant until runbooks/Deep Research drive the rewrite path. |  
| **Q12** Rollback | backup + git tag + restore. | Archive-first remains absolute. Node/old runtime material can be rollback/reference evidence, but any repo merge, delete, rename, or archive-only transition must preserve FD and PE identity/memory and wait for Shane-approved plan. |  
