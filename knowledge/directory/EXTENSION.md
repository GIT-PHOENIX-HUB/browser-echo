  ...
  - - Auto-inject text into claude.ai conversations (human stays in the loop)
    - - Auto-commit without user action (reminders only)
      ...
      - 1. **Side Panel** — Persistent UI via Chrome Side Panel API. 4 tabs: Bootstrap, Mission, Checkpoint, Handoff.
        2. 2. **Background Service Worker** — GitHub API integration, checkpoint timer (default 10min), tab close listener, session tracking.
           3. 3. **Content Script** — claude.ai only. Detects page, adds "BBB" indicator. Read-only, no text injection.
              ...
              13. ### Extension Source (in browser-echo)
